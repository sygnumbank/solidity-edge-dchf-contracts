const { getAdmin, getImplementation, encodeCall, expectEvent, expectRevert, assertRevert, EdgeToken, EdgeTokenFixProxyStorageUpgrade, EdgeTokenProxy, ZERO_ADDRESS } = require('../../common')
const { BaseOperators, Whitelist, TraderOperators, BlockerOperators } = require('@sygnum/solidity-base-contracts')


contract('EdgeTokenFixProxyStorageUpgrade', ([owner, admin, operator, proxyAdmin, proxyAdminNew, attacker, whitelisted]) => {
    beforeEach(async () => {
        ...
        this.tokenImplUpgrade = await EdgeTokenFixProxyStorageUpgrade.new()
    ...
    context('deployed proxy', () => {
        ...
        context('upgradability', () => {
            describe('upgrade to', () => {
                beforeEach(async () => {
                    this.initializeAllExternalContractsData = encodeCall('initializeAllExternalContracts', ['address', 'address', 'address', 'address'], [this.whitelist.address, this.baseOperators.address, this.blockerOperators.address, this.traderOperators.address])
                });
                ...
                describe('upgrade and call', () => {
                    describe('functional', () => {
                        describe('from proxy admin', () => {
                            beforeEach(async () => {
                                await this.proxy.upgradeToAndCall(this.tokenImplUpgrade.address, this.initializeAllExternalContractsData, { from: proxyAdmin })
                            });
                    ...
                    describe('non-functional', () => {
                        it('reverts from token admin', async () => {
                            await assertRevert(this.proxy.upgradeToAndCall(this.tokenImplUpgrade.address, this.initializeAllExternalContractsData, { from: admin }))
                        });
                        it('reverts when implementation empty address', async () => {
                            await assertRevert(this.proxy.upgradeToAndCall(ZERO_ADDRESS, this.initializeAllExternalContractsData, { from: admin }))
                        });
                    });
                })
                context('upgrade and call', () => {
                    ...
                    describe('set token instance', () => {
                        beforeEach(async () => {
                            this.token = await EdgeTokenFixProxyStorageUpgrade.at(this.proxy.address)
                        });
                        ...
                        it('baseOperators pointer updated', async () => {
                            assert.equal(await this.token.getOperatorsContract(), this.baseOperators.address)
                        });
