...
contract('EdgeTokenBlockUnblockTraderUpgrade', ([owner, admin, operator, proxyAdmin, proxyAdminNew, attacker, whitelisted]) => {
    ...
    context('deployed proxy', () => {
        ...
        context('upgradability', () => {
            describe('upgrade to', () => {
                beforeEach(async () => {
                    this.initializeBlockerTraderOperatorsData = encodeCall('initializeBlockerTraderOperators', ['address', 'address'], [ this.blockerOperators.address, this.traderOperators.address ])
                });
