					...
					describe('non-batch', async () => {
						describe('non-functional', async () => {
							it('revert confiscatees and values not equal', async () => {
								await expectRevert(this.token.batchConfiscate(TWO_ADDRESSES, TWO_ADDRESSES, [MINT], { from: operator }), 'EdgeToken: values and recipients are not equal')
							})
							it('revert receivers and values not equal', async () => {
								await expectRevert(this.token.batchConfiscate(TWO_ADDRESSES, [TWO_ADDRESSES[0]], [MINT, MINT], { from: operator }), 'EdgeToken: values and recipients are not equal')
							})
